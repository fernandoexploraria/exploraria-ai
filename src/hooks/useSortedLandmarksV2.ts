...
interface LandmarkWithDistanceV2 {
  landmark: Landmark;
  distance: number;
}

export const useSortedLandmarksV2 = (
  userLocation: UserLocation | null,
  landmarks: Landmark[],
  maxDistance: number,
  debugOverrides?: DebugOverrides
): LandmarkWithDistanceV2[] => {
  const sortedLandmarks = useMemo(() => {
    ...
    const landmarksWithDistance = landmarks.map(landmark => {
      ...
      if (debugOverrides?.enabled && debugOverrides.targetLandmarkId === landmark.id && debugOverrides.forcedDistance !== null) {
        ...
        console.log(`🔧 [V2] Debug Override: ${landmark.name} distance forced to ${distance}m`);
      } else {
  ...
  console.log(`🗺️ [V2] useSortedLandmarksV2: ${landmarks.length} total landmarks, ${sortedLandmarks.length} within ${maxDistance}m range`);
