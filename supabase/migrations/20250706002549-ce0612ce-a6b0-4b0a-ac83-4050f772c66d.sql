...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_generated_landmarks_tour_place 
ON generated_landmarks(tour_id, place_id);

-- Add index for chronological queries on generated_landmarks
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_generated_landmarks_created_at 
ON generated_landmarks(created_at DESC);

-- Add index for better performance on landmark queries by place_id
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_generated_landmarks_place_id 
