...
-- Create a function to clean up all user data and records
...
BEGIN
  ...
  SELECT COUNT(*) INTO voice_interactions_count FROM public.voice_interactions;
  ...
  RETURN FORMAT('Cleanup completed successfully. Deleted: %s voice interactions, %s tour stats, %s subscribers, %s users', 
                voice_interactions_count, tour_stats_count, subscribers_count, users_count);
END;
$$;
